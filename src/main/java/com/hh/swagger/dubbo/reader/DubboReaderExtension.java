package com.hh.swagger.dubbo.reader;
...
//		}
        ...
        try {
            ...
            aClass = Class.forName("com.hundsun.jrescloud.rpc.annotation.CloudFunction");
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        }
        Annotation annotation = ReflectionUtils.getAnnotation(method, (Class<? extends Annotation>) aClass);
        if (annotation != null) {
            InvocationHandler h = Proxy.getInvocationHandler(annotation);
            Field memberValues = null;
            try {
                memberValues = h.getClass().getDeclaredField("memberValues");
            ...
            try {
                map = (LinkedHashMap) memberValues.get(h);
...
//		}
        schemes.addAll(parseSchemes("http,https"));
    ...
    public void applyTags(ReaderContext context, Operation operation, Method method) {
        ...
        ReferenceManager.getInstance().addAllService(tags);
    }
    ...
    private void applyParametersV2(ReaderContext context, Operation operation, String name,
        ...
        final Parameter parameter = readParam(context.getSwagger(), type, cls, null == apiParam ? null : (ApiParam) apiParam);
        if (parameter != null) {
